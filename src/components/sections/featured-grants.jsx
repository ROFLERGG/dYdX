      ...
      <div className="container max-lg:mx-0 max-lg:px-0">
        <div className="flex flex-col">
          ...
          <div className="relative overflow-x-hidden">
            <div className="relative snap-x snap-mandatory overflow-x-scroll scrollbar-none flex">
              {isLoading &&
                Array(4)
                  .fill(0)
                  .map((_, id) => {
              ...
              {slicedPosts.map((post) => {
                return <GrantCard key={post.id} id={post.id} className={`px-6 shrink-0 snap-center max-w-[460px] max-sm:max-w-[400px] max-[460px]:max-w-[360px]`} {...post} />;
              })}
            </div>
          </div>
        </div>
