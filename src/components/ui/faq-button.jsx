...
const AccordionButton = ({question, answer, id}) => {
  ...
  return (
    <div>
      ...
      <label onClick={setIsOpenHandler} htmlFor={id + 1} className={`flex flex-col justify-center ${isOpen ? "space-y-6" : ""} py-6 cursor-pointer select-none`}>
        <div className="flex justify-between items-center space-x-6">
          ...
          <div className={`w-4 h-4 relative items-center shrink-0`}>
            ...
            <div className={`absolute inset-x-1/2 inset-y-1/2 -translate-x-1/2 -translate-y-1/2 w-0.5 bg-white-100 rounded-full duration-100 ${isOpen ? "h-0" : "h-full"}`}></div>
          </div>
        </div>
        <div className={`${isOpen ? "max-h-96" : "max-h-0 overflow-hidden"}`}>
