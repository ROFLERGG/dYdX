...
//   }
// ]
const GrantCard = forwardRef(({ id, image, category, title, amount, description, team, className = '' }, ref) => {
  return (
    <div ref={ref} id={id} key={id} className={'flex min-h-[500px] ' + className}>
      <Link to={`/grants/${id}`} className={`flex ${image ? 'flex-col' : ''} flex-1 bg-secondary rounded-2xl hover:bg-secondaryHover duration-150 ease-in-out`}>
        {/* image */}
        {image && <div className="w-full min-h-[180px] bg-[#232334] rounded-2xl"></div>}
        {/* content */}
        <div className="p-6 flex flex-1 flex-col justify-between space-y-4">
          ...
          <div className="flex items-center -space-x-4">
            {team.map((user, id) => {
              return (
                <div key={id} className="w-[40px] h-[40px] bg-secondaryHover ring-2 ring-secondary rounded-full">
                  {id}
                </div>
              );
            })}
